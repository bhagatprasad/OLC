...
namespace OLC.Web.API.Manager
{
    public class PaymentOrderManager : IPaymentOrderManager
    {
        ...
        public async Task<PaymentOrderDetails> GetExecutivePaymentOrderDetailsAsync(long paymentOrderId)
        {
            ...
            if (paymentOrderId > 0)
            {
                var depositeOrder = await _depositOrderManager.GetDepositOrderByOrderIdAsync(paymentOrderId);
        ...
        public async Task<List<ExecutivePaymentOrders>> GetOrdersForAutoApprovalAsync(long transactionFeeId)
        {
            List<ExecutivePaymentOrders> userPaymentOrders = new List<ExecutivePaymentOrders>();
            ...
            SqlCommand cmd = new SqlCommand("[dbo].[uspGetOrdersForAutoApproval]", conn);

            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.AddWithValue("@transactionFeeId", transactionFeeId);
        ...
        public async Task<List<PaymentOrder>> GetApprovedOrdersForDepositAsync()
        {
            List<PaymentOrder> getPaymentOrders = new List<PaymentOrder>();
            ...
            SqlCommand sqlCommand = new SqlCommand("[dbo].[uspGetApprovedOrdersForDeposit]", connection);
